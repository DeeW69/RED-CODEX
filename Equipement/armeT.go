...
func ChargerArmesTroll() (map[string]ArmeTroll, error) {
	file, err := os.ReadFile("Equipement/aa_equipement.json")
	if err != nil {
		return nil, fmt.Errorf("cannot read JSON file: %v", err)
	}

	var data map[string]map[string]map[string]ArmeTroll
	err = json.Unmarshal(file, &data)
	if err != nil {
		return nil, err
	}

	armes, exists := data["armes"]["troll"]
	if !exists {
		return nil, fmt.Errorf("armes troll not found in JSON")
	}

	return armes, nil
}

func AfficherArmeTroll(arme ArmeTroll) {
