...
func ChargerArmesGrimline() (map[string]ArmeGrimline, error) {
	file, err := os.ReadFile("Equipement/aa_equipement.json")
	if err != nil {
		return nil, fmt.Errorf("cannot read JSON file: %v", err)
	}

	var data map[string]map[string]map[string]ArmeGrimline
	err = json.Unmarshal(file, &data)
	if err != nil {
		return nil, err
	}

	armes, exists := data["armes"]["grimeline"]
	if !exists {
		return nil, fmt.Errorf("armes grimline not found in JSON")
	}

	return armes, nil
}

func AfficherArmeGrimline(arme ArmeGrimline) {
